...
# First steps using the subset function. 
# Run this file then run renameAddin().
# Make test box rename column instead. Update dataframe preview (see ui and server files in this dir. Done button should return dataframe and list of all commands to rename columns.)

#' Rename columns
#' Based on Joe Cheng's Subset a Data Frame.
#'
...
renameAddin <- function() {
  ...
  ui <- miniPage(
    gadgetTitleBar("Rename data.frame columns"),
    miniContentPanel(
      stableColumnLayout(
        textInput("data", "Data"),
